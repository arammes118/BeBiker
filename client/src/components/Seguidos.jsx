...
export const Seguidos = () => {
    ...
    useEffect(() => {
        async function ver() {
            const pet = await peticion('/seguidos/ver?id=' + perfil_id)
            if (Array.isArray(pet)) {
                const listaActualizada = await Promise.all(pet.map(async (seguido) => {
                    const imageObject = handleBase64Image(seguido.foto)
                    return {
                        ...seguido,
                        foto: imageObject
                    }
